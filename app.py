...
def export_pdf_neraca(df_aset, df_kewajiban, df_ekuitas, total_aset, total_kewajiban, total_ekuitas, nama_pt, pejabat, tanggal_akhir):
    buf = BytesIO()
    c = canvas.Canvas(buf, pagesize=A4)
    w, h = A4
    y = h - 2.5*cm
...
if coa_file and saldo_file and jurnal_file:
    ...
    if "saldo_awal" not in saldo_awal.columns:
        ...
        if candidates:
            ...
            st.error(f"Kolom saldo tidak ditemukan. Kolom tersedia: {list(saldo_awal.columns)}")
            st.stop()
    ...
    # Ekuitas: kredit normal, jika debit => negatif
    ...
    for title, data, total in [("ASET", df_aset, total_aset), ("KEWAJIBAN", df_kewajiban, total_kewajiban), ("EKUITAS", df_ekuitas, total_ekuitas)]:
        st.subheader(title)
        if mode == "Detail":
            col = "saldo_akhir_adj" if title=="EKUITAS" else "saldo_akhir"
            st.dataframe(data[["kode_akun","nama_akun",col]])
